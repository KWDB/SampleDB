...
kwdb_copy_smart_meter_data_to_container() {
  local data_path="${KWDB_LOCAL_DATA_PATH%/}"
  local source_rdb="$data_path/extern/rdb"
  local source_tsdb="$data_path/extern/tsdb"
  local target_store
  target_store=$(kwdb_resolve_container_store)
  target_store="${target_store%/}"

  if [ ! -d "$source_rdb" ] || [ ! -d "$source_tsdb" ]; then
    kwdb_die "未找到示例数据目录: $data_path/extern/rdb 或 $data_path/extern/tsdb，请先执行 prepare_data.sh 或通过 --data-path 指定数据目录"
  fi

  kwdb_ensure_container_ready
  docker exec "$KWDB_CONTAINER_NAME" sh -lc 'mkdir -p "$1/extern" && rm -rf "$1/extern/rdb" "$1/extern/tsdb"' sh "$target_store"
  docker cp "$source_rdb" "$KWDB_CONTAINER_NAME:$target_store/extern/"
  docker cp "$source_tsdb" "$KWDB_CONTAINER_NAME:$target_store/extern/"
}
