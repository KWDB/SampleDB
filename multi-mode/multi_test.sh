...
usage() {
  cat <<EOF
用法:
  bash multi_test.sh [listen_port] [http_port] [store]
  bash multi_test.sh --container <container_name> [--port container_sql_port] [--data-path local_data_path] [--container-store container_store]
