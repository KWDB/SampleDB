...
kwdb_copy_extern_to_container() {
  local data_path="${KWDB_LOCAL_DATA_PATH%/}"
  local source_dir="$data_path/extern"
  local target_store
  ...
  kwdb_ensure_container_ready
  docker exec "$KWDB_CONTAINER_NAME" mkdir -p "$target_store"
  docker cp "$source_dir" "$KWDB_CONTAINER_NAME:$target_store/"
}
